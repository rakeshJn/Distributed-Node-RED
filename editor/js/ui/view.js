...
RED.view = (function() {
    ...
    function generateLinkPath(origX,origY, destX, destY, sc) {
        ...
        if (dx*sc > 0) {
            ...
            var topY = dy>0?Math.min((destY + midY)/2 - dy/2 , origY+node_height/2):Math.max((destY + midY)/2 - dy/2 , origY-node_height/2);

            var bottomX = destX - sc*node_width*scale;
            var bottomY = dy>0?Math.max((destY + midY)/2, destY-node_height/2):Math.min((destY + midY)/2, destY+node_height/2);

            var scy = dy>0?1:-1;
            var cp = [
                // Orig -> Top
                [(origX+topX)/2,origY],
                [topX,dy>0?Math.max(origY, topY-node_height/2):Math.min(origY, topY+node_height/2)],
                // [topX,origY],

                // Top -> Mid
                [topX,dy>0?Math.min(midY, topY+node_height/2):Math.max(midY, topY-node_height/2)],
                [(origX+topX)/2,dy>0?Math.min(midY, topY+node_height/2):Math.max(midY, topY-node_height/2)],

                // Mid -> Bottom
                [(destX+bottomX)/2,dy>0?Math.max(midY, bottomY-node_height/2):Math.min(midY, bottomY+node_height/2)],
                [bottomX,dy>0?Math.max(midY, bottomY-node_height/2):Math.min(midY, bottomY+node_height/2)],

                // Bottom -> Dest
                [bottomX, dy>0?Math.min(destY, bottomY+node_height/2):Math.max(destY, bottomY-node_height/2)],
                // [bottomX,destY],
                [(destX+bottomX)/2,destY]

            ]

            if (cp[3][1] === topY+scy*node_height/2) {
                cp[1][1] = topY-scy*node_height/4;
                cp[2][1] = topY+scy*node_height/4;
                cp[3][0] = topX; //(origX+topX)/2 + (topX - origX)
            }

            if (cp[4][1] === bottomY-scy*node_height/2) {
                cp[5][1] = bottomY-scy*node_height/4;
                cp[6][1] = bottomY+scy*node_height/4;
                cp[4][0] = bottomX; //(origX+topX)/2 + (topX - origX)
            }
            ...
            return "M "+origX+" "+origY+
               ...
               " C "+
                ...
                " C "+
                    ...
                    //
                    //
                    //  drawPoint(origX,origY)+ drawPoint(cp[0][0],cp[0][1],3)+ drawPoint(cp[1][0],cp[1][1],3)+
                    //  drawPoint(topX,topY)+ drawPoint(cp[2][0],cp[2][1],3)+ drawPoint(cp[3][0],cp[3][1],3)+
                    //  drawPoint(midX,midY)+ drawPoint(cp[4][0],cp[4][1],3)+ drawPoint(cp[5][0],cp[5][1],3)+
                    //  drawPoint(bottomX,bottomY)+ drawPoint(cp[6][0],cp[6][1],3)+ drawPoint(cp[7][0],cp[7][1],3)+
                    //  drawPoint(destX,destY)
